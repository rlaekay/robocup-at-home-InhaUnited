...
public:
  ...
  double sensorNoiseR = 1.0;
  int minMarkerCnt = 3;
  bool enableLog = true;
  // string logIP = "127.0.0.1:9876";

  // const rerun::RecordingStream log =
  //     rerun::RecordingStream("locator", "locator");
  ...
  void init(FieldDimensions fd, int minMarkerCnt = 4,
            double residualTolerance = 0.4, double muOffsetParam = 2.0,
            bool enableLog = true, string logIP = "127.0.0.1:9876");

  void calcFieldMarkers(FieldDimensions fd);

  LocateResult locateRobot(vector<FieldMarker> markers_r, PoseBox2D constraints,
                           int numParticles = 200, double offsetX = 2.0,
  ...
  inline double probDesity(double r, double mu, double sigma) {
    ...
    return 1 / sqrt(2 * M_PI * sigma * sigma) *
           exp(-(r - mu) * (r - mu) / (2 * sigma * sigma));
  };

  void logParticles(double time_sec);
};
...
public:
  ...
  static PortsList providedPorts() {
    return {
        InputPort<string>(
            "mode", "enter_field",
            "must be one of [trust_direction, face_forward, fall_recovery]"),
        InputPort<double>("msecs_interval", 10000,
                          "防止过于频繁地校准, "
                          "如果上一次校准距离现在小于这个时间, 则不重新校准."),
    };
  };
...
public:
  SelfLocateEnterField(const string &name, const NodeConfig &config,
                       Brain *_brain)
      : SyncActionNode(name, config), brain(_brain) {}
...
public:
  ...
  static PortsList providedPorts() {
    return {
        ...
        InputPort<double>(
            "max_dist", 2.0,
            "marker 距离机器人的距离小于此值时, 才进行校准. (距离小测距更准)"),
        InputPort<double>(
            "max_drift", 1.0,
            "校准后的位置与原位置距离应小于此值, 否则认为校准失败"),
        InputPort<bool>("validate", true,
                        "校准后, 用其它的 marker 进行验证, 要求小于 locator 的 "
                        "max residual"),
    };
  };
...
public:
  ...
  static PortsList providedPorts() {
    return {
        ...
        InputPort<double>(
            "max_dist", 2.0,
            "border 距离机器人的距离小于此值时, 才进行校准. (距离小测距更准)"),
        InputPort<double>(
            "max_drift", 1.0,
            "校准后的位置与原位置距离应小于此值, 否则认为校准失败"),
        InputPort<bool>("validate", true,
