...
/**
 * Brain 核心类，因为要传递智能指针给 BrainTree，所以需要继承自
 * enable_shared_from_this 数据封闭到各个子对象中，不要直接存在在 Brain 类
 * 如果是静态的配置值，放到 config
 * 运行时的动态数据，放到 data
 * TODO:
 * BehaviorTree 的 blackboard
 * 里也存了一些数据，看看是不是有些重复存储了的，可以考虑去掉,
 * 目前的设计里，brain 指针传到了 BehaviorTree 的结点里了，在那里直接访问
 * brain->config 和 brain->data
 */
...
public:
  ...
  // 初始化操作，只需要在 main
  // 函数中调用一次，初始化过程如不符合预期，可以抛异常直接中止程序
  void init();
   ...
   *
   * @param  margin double, 计算角度时,
   * 返回值比实际的球门的位置向内移动这个距离, 因为这个角度球会被门柱挡住.
   * 此值越大, 射门准确性越高, 但调整角度花的时间也越长.
   *
   * @return vector<double> 返回值中 vec[0] 是左门柱, vec[1] 是右门柱.
   * 注意左右都是以对方的方向为前方.
   */
  ...
  bool isBoundingBoxInCenter(BoundingBox boundingBox, double xRatio = 0.8,
                             double yRatio = 0.8);
  ...
  void playSound(string soundName, double blockMsecs = 1000,
  ...
  vector<double> findSafeDirections(double startAngle, double safeDist,
...
private:
  ...
  int markCntOnFieldLine(const string MarkType, const FieldLine line,
  ...
  vector<GameObject>
  getGameObjects(const vision_interface::msg::Detections &msg);
...
//   void logDebugInfo();
  ...
  rclcpp::Subscription<game_controller_interface::msg::GameControlData>::
      SharedPtr gameControlSubscription;
  rclcpp::Subscription<vision_interface::msg::Detections>::SharedPtr
      detectionsSubscription;
  rclcpp::Subscription<vision_interface::msg::LineSegments>::SharedPtr
      subFieldLine;
  rclcpp::Subscription<booster_interface::msg::Odometer>::SharedPtr
      odometerSubscription;
  rclcpp::Subscription<booster_interface::msg::LowState>::SharedPtr
      lowStateSubscription;
  rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr imageSubscription;
  rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr
      depthImageSubscription;
  rclcpp::Subscription<geometry_msgs::msg::Pose>::SharedPtr
      headPoseSubscription;
  rclcpp::Subscription<booster_interface::msg::RawBytesMsg>::SharedPtr
